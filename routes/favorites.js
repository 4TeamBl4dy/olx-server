...
const Favorite = require('../models/Favorite'); // Импортируем модель Favorite
...
router.get('/', async (req, res) => {
    try {
        const favorites = await Favorite.find()
            .populate('userId', 'name email') // Получаем данные пользователя (имя и email)
            .populate('productId', 'title price'); // Получаем данные объявления (название и цена)
